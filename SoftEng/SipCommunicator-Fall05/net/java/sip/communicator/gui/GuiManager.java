...
implements GuiCallback
{
	...
	void payButton_actionPerformed(ActionEvent evt)
	{
		Double amount = Double.parseDouble(manageSplash.payField.getText());
		if (amount == null) {
			return;
		}

		for (int i = listeners.size() - 1; i >= 0; i--) {
			( (UserActionListener) listeners.get(i)).handlePayRequest(amount);
		}

	}

	void unblockingButton_actionPerformed(ActionEvent evt)
	{
		String unblocked = manageSplash.userNameTextField.getText();
		if (unblocked == null || unblocked.trim().length() < 1) {
			return;
		}

		for (int i = listeners.size() - 1; i >= 0; i--) {
			( (UserActionListener) listeners.get(i)).handleUnblockRequest(unblocked);
		}
	}

	void unforwardButton_actionPerformed(ActionEvent evt)
	{
		String unforward = manageSplash.userNameTextField.getText();
		if (unforward == null || unforward.trim().length() < 1) {
			return;
		}
